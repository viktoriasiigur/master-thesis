...
2. Impute the sensor
3. Switch main sensors imputation with the median value of main and nearest. (round median values up)
...
def get_nearest_indices(query_point):
    ...
    distances, nearest_indices = sensor_positions_tree.query(query_point_in_radians, k=2)
...
def generate_nearest_imputed_data():
    for location, name in sensor_positions.values:
        main_df = get_main_df(name)
        distances, nearest_indices = get_nearest_indices(location)
        # for nearest_index in nearest_indices[0]:
        ...
        # main_df.to_csv(f'imputed_data/nearest-k-1/{name}-nearest.csv')
        distance_df = pd.DataFrame({'name': [name], 'nearest_name': [get_nearest_name(nearest_indices[0][1])], 'nearest_meters': np.round([distances[0][1] * EARTH_RADIUS_METERS])}).set_index('name')
        distance_df.to_csv('imputed_data/nearest_distance.csv', mode='a', header=None)
